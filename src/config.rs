...
impl Config {
    ...
    /// Priority: FIREPLACE_API_CONFIG env var > /etc/fireplace-api/{room}.toml > ./config/{room}.toml > defaults
    pub fn load_with_fallback(room: &str) -> Self {
        // Try environment variable first
        if let Ok(config_path) = std::env::var("FIREPLACE_API_CONFIG") {
            match Self::load(&config_path) {
                ...
                Err(e) => {
                    tracing::warn!("Failed to load config from FIREPLACE_API_CONFIG ({}): {}", config_path, e);
                }
            }
        }

        // Try /etc/fireplace-api/{room}.toml
        let etc_path = format!("/etc/fireplace-api/{}.toml", room);
        match Self::load(&etc_path) {
            Ok(cfg) => {
                tracing::info!("Configuration loaded from {}", etc_path);
                return cfg;
            }
            Err(_) => {
                tracing::debug!("Config not found at {}", etc_path);
            }
        }

        // Try ./config/{room}.toml
        let local_path = format!("config/{}.toml", room);
        match Self::load(&local_path) {
            Ok(cfg) => {
                tracing::info!("Configuration loaded from {}", local_path);
                return cfg;
            }
            Err(_) => {
                tracing::debug!("Config not found at {}", local_path);
            }
        }
